...
class StandardKalmanFilter:
    def __init__(self, x, y, dt=1.0):
        ...
        # FIXED: Higher process noise for better adaptation
    ...
    def predict(self):
        ...
        max_velocity_x = 8
        max_velocity_y = 15
        vx = np.clip(vx, -max_velocity_x, max_velocity_x)
        vy = np.clip(vy, -max_velocity_y, max_velocity_y)
        
        # Dampen velocities
        pred_x = pred_x + vx * 0.2
        pred_y = pred_y + vy * 0.6
        
        # Periodic velocity reset
        self.velocity_reset_counter += 1
        if self.velocity_reset_counter % 15 == 0:
            self.kf.statePost[2][0] *= 0.3
            self.kf.statePost[3][0] *= 0.7
...
def detect_person_motion(frame, prev_frame):
    ...
    # Stronger blur for distant motion
    ...
    for cnt in contours:
        ...
        # Lower minimum area for distant person
        ...
        if h > 15 and 1.2 < aspect_ratio < 4.5 and w > 10:
            cx, cy = x + w//2, y + h//2
...
def detect_background_subtraction(frame, bg_subtractor):
    """Background subtraction with aspect ratio filtering"""
    # Higher learning rate for dynamic background
    ...
    for cnt in contours:
        area = cv2.contourArea(cnt)
        # Lower minimum for distant detection
        if area < 180 or area > 45000:
            continue
        ...
        if h > 20 and 1.2 < aspect_ratio < 5.0 and w > 15:
            cx, cy = x + w//2, y + h//2
...
def combine_detections(all_dets):
    """Combine nearby detections into single detections"""
    if not all_dets:
        return []
    ...
    for i, det1 in enumerate(all_dets):
        ...
        for j, det2 in enumerate(all_dets[i+1:], i+1):
            ...
            # Larger merge distance to combine fragments
            if dist < 180:
                similar.append(det2)
                used.add(j)
        ...
        if similar:
            ...
            # Create bounding box that encompasses ALL detections
            all_x = [d['bbox'][0] for d in similar]
            all_y = [d['bbox'][1] for d in similar]
            all_x2 = [d['bbox'][0] + d['bbox'][2] for d in similar]
            all_y2 = [d['bbox'][1] + d['bbox'][3] for d in similar]
...
def smooth_bbox(bbox, history, max_size=4):
    ...
    if len(history) >= 2:
        ...
        # Enforce aspect ratio 1.5:1 to 3:1 (height:width)
        ...
        # Wider size range for distance variation
...
print("="*70)
print("FIXED PERSON TRACKER - CORRECTED VERSION")
print("="*70)
print("\n🎯 VISUALIZATION:")
...
while True:
    ...
    if not tracking_init:
        ...
        # Filter out bad aspect ratios and tiny boxes
    ...
    # PHASE 2: TRACKING MODE
        ...
        # Adaptive search radius based on lost frames
        if all_dets:
            best_det = None
            best_dist = float('inf')
            
            search_radius = 250 + (lost_frames * 15)
            search_radius = min(search_radius, 600)
        ...
        # Force correction if many detections but none matched
