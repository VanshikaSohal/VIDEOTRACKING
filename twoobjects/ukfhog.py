...
class UKFTracker:
    def __init__(self, x, y, tracker_id, dt=1.0):
        ...
        self.ukf.P *= 10.0
        self.ukf.R  = np.eye(2) * 6.0
        self.ukf.Q  = np.eye(4) * 0.03

    def predict(self):
        self.ukf.predict()
        px, py = int(self.ukf.x[0]), int(self.ukf.x[1])
...
while True:
    ...
    # --- Full frame detections for initialisation ---
    ...
    for t_idx, t in enumerate(trackers):
        ...
        cv2.putText(frame, f"UKF Obj {t.id}",
    ...
    prev_frame = frame.copy()
    cv2.imshow("UKF Dual Tracking", frame)
