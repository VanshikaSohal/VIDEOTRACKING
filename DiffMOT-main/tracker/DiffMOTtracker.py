...
class STrack(BaseTrack):
    ...
    def update(self, new_track, frame_id, update_feature=False):
        ...
        :return:
        """
        print("TRACKER UPDATE CALLED", flush=True)
        self.frame_id = frame_id
        self.tracklet_len += 1
        ...
        print("[DEBUG] track_id:", self.track_id)
...
class diffmottracker(object):
    ...
    def update(self, dets_norm, model, frame_id, img_w, img_h, tag, img=None):
        ...
        # expected: frame, x1, y1, w, h, score (or similar)
        
        print("FRAME:", frame_id, "DETECTIONS:", dets_norm.shape)
        print("FIRST DETS:", dets_norm[:3])
        print("[DEBUG] raw dets shape:", dets.shape)
        print("[DEBUG] sample dets:", dets[:3])
        print("[DEBUG] high_thres:", self.det_thresh)
        print("[DEBUG] low_thres:", self.config.low_thres)
        if dets.shape[1] > 5:
           dets = dets[:, -5:]   # take last 5 columns safely
        # convert tlwh -> tlbr
        dets[:, 2] = dets[:, 0] + dets[:, 2]
        dets[:, 3] = dets[:, 1] + dets[:, 3]
        remain_inds = dets[:, 4] > 0.0
        inds_low = dets[:, 4] > self.config.low_thres
        inds_high = dets[:, 4] < self.det_thresh
        inds_second = np.logical_and(inds_low, inds_high)
        dets_second = dets[inds_second]
        dets = dets[remain_inds]
        print("[DEBUG] detections after filtering:", dets.shape)
