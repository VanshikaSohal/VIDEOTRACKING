...
def draw_tracks(frame, tracks, seq_name, frame_id, total_frames):
    ...
    cv2.putText(frame, "YOLOv8 + BoT-SORT (Swin Transformer ReID)",
                (w-600, h-15), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0,255,128), 2, cv2.LINE_AA)
    for trk in tracks:
...
def run_tracking_and_video():
    ...
    video_path = os.path.join(OUTPUT_ROOT, "botsort_swin_tracking.mp4")
    ...
    for seq in SEQS:
        ...
        frames = sorted([f for f in os.listdir(img_dir) if f.endswith('.jpg') or f.endswith('.png')])
        if total_frames == 0:
...
def run_evaluation(tracker_name):
    ...
    results_path = os.path.join(TRACKEVAL_DIR,"data","trackers","mot_challenge","MOT17-train",tracker_name,"pedestrian_summary.txt")
...
if __name__ == "__main__":
    print("\n"+"="*60)
    print("  YOLOv8 + BoT-SORT (Swin Transformer) — MOT17")
    print("="*60)
